...
test('bad and legacy saves reset safely to the supplied fresh run', () => { const fresh = state(); assert.equal(loadRunState('{bad json', fresh), fresh); assert.equal(loadRunState(JSON.stringify({ schemaVersion: 0 }), fresh), fresh); });
