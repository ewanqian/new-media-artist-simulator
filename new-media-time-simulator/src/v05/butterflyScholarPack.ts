...
export const butterflyScholarIdentity = {
  title: '哥斯达黎加的蝴蝶学者',
  role: '艺术家 / 蝴蝶研究者',
  practice: '以蝴蝶、寄主植物、空间扫描和长期观察记录为材料，研究一个地点如何被记住，又如何在被数字化以后继续变化。',
  priorWork: [
    '把城市边缘植物做成带时间戳的扫描档案',
    '用植物标本、现场录音和点云做过一件小型空间作品',
    '长期收集“没有发生”的观察记录：没有出现的物种、失败的扫描、缺失的标签'
  ],
  currentQuestion: '如果保存不是把一个环境冻结下来，那么数字化到底应该保存什么？'
};

export const butterflyScholarTraining: ButterflyTrainingStep[] = [
  {
    id: 'train-choose-subject',
    title: '先决定你在采集什么',
...
export const butterflyScholarNarrativePack: NarrativePack = {
  ...
  actors: [
    {
      id: 'player-butterfly-scholar',
      name: '你',
      publicRole: '艺术家 / 蝴蝶研究者',
      motives: ['完成一套关于情感空间数字化的新作品', '理解数字保存到底保留什么', '把野外方法带回自己的长期实践'],
      remembers: []
    },
    {
      id: 'advisor-rui',
      name: '睿',
      publicRole: '工作室艺术顾问',
      motives: ['让你离开熟悉的工作室', '逼你把扫描从技术手段变成真正的方法'],
      remembers: ['你是否愿意进入不确定现场', '你回来后有没有只带回漂亮素材']
    },
    {
      id: 'ines',
      name: 'Inés',
      publicRole: '独立艺术组织的现场协调人',
      coverStory: '负责研究站、交通、翻译和植物档案对接。',
      hiddenRole: '同时代表另一个艺术研究网络评估这片区域的数据、档案和未来合作权。她并没有在第一次见面时说完整。',
      motives: ['保护研究站的一部分档案', '完成自己所属组织的评估', '判断你是否只是来提取视觉素材', '在合作中逐渐形成私人感情'],
      remembers: ['你第一次是否直接追问她的身份', '你如何处理不能带走的材料', '你是否把她的私人记录放进公开作品', '你们第一次真正互相信任发生在哪里']
    },
    {
      id: 'rojas',
      name: 'Rojas',
      publicRole: '研究站技术员 / 采集协助',
      motives: ['保证采集不破坏长期样地', '让你别把所有技术问题都解释成艺术选择'],
      remembers: ['你有没有听采集建议', '你是否诚实记录失败扫描']
    }
  ],
  facts: [
    { id: 'fact-invite', subjectId: 'route', label: '你的工作室收到一份热带研究站短期驻留邀请。', state: 'verified', sourceIds: ['advisor-rui'] },
  ...
  nodes: [
    {
      id: 'bs-01-invite', sceneId: 'scene-studio', speakerId: 'advisor-rui', channel: 'message',
      text: ['你的艺术顾问发来一段很短的语音。', '“别再用旧扫描做第三个版本了。这里有一个研究站邀请。去现场，回来以后再告诉我什么值得保存。”'],
      choices: [
        { id: 'bs-go', label: '接受邀请，先去看看', subtext: '不要求先知道作品最终是什么。', effects: { setFlags: ['accepted-field-trip'], memories: [{ actorId: 'advisor-rui', about: 'departure', value: '没有要求先确定作品形式', weight: 2 }] }, nextNodeId: 'bs-02-arrival' },
        { id: 'bs-go-with-question', label: '接受，但先写下一句话', subtext: '“如果保存不是冻结，那我究竟在保存什么？”', effects: { setFlags: ['accepted-field-trip', 'question-before-trip'], memories: [{ actorId: 'advisor-rui', about: 'departure', value: '带着明确问题进入现场', weight: 3 }] }, nextNodeId: 'bs-02-arrival' }
      ]
    },
    {
      id: 'bs-02-arrival', sceneId: 'scene-arrival', speakerId: 'ines', channel: 'dialogue',
      text: ['Inés 在研究站入口等你。她准确说出了你两年前那件几乎没人看过的植物扫描作品。', '她说自己只是负责现场协调，但她随身文件上的组织标记与你收到的邀请函并不一样。'],
      choices: [
        { id: 'bs-ask-now', label: '现在就问她', subtext: '“你到底替谁工作？”', effects: { setFlags: ['asked-ines-directly'], factUpdates: [{ factId: 'fact-ines-role-public', state: 'contradicted', sourceId: 'player' }, { factId: 'fact-ines-network', state: 'observed', sourceId: 'documents' }], trustDelta: { ines: -1 }, memories: [{ actorId: 'ines', about: 'first-meeting', value: '玩家第一时间质疑了她的身份', weight: 3 }] }, nextNodeId: 'bs-03-field' },
        { id: 'bs-hold-question', label: '先记下来，不追问', subtext: '先看她之后实际怎么做。', effects: { setFlags: ['held-identity-question'], factUpdates: [{ factId: 'fact-ines-network', state: 'observed', sourceId: 'documents' }], trustDelta: { ines: 1 }, memories: [{ actorId: 'ines', about: 'first-meeting', value: '玩家发现异常但没有立刻逼问', weight: 2 }] }, nextNodeId: 'bs-03-field' }
      ]
    },
    {
    ...
    {
      id: 'bs-04b-reveal', sceneId: 'scene-night', speakerId: 'ines', channel: 'dialogue',
      text: ['她终于把另一张证件放在桌上。她确实属于邀请你的艺术组织，但也同时替另一个研究网络工作。', '后者想知道：这片区域未来产生的数据，谁有资格保存、展示、复制。'],
      choices: [
        { id: 'bs-reveal-accept', label: '听她把话说完', subtext: '合作继续，但数据权利成为项目内容。', effects: { setFlags: ['ines-revealed', 'relationship-open'], factUpdates: [{ factId: 'fact-ines-network', state: 'revealed', sourceId: 'ines' }, { factId: 'fact-data-rights', state: 'verified', sourceId: 'ines' }], trustDelta: { ines: 2 }, memories: [{ actorId: 'ines', about: 'reveal', value: '玩家允许她完整解释后再决定', weight: 4 }] }, nextNodeId: 'bs-05-public' },
        { id: 'bs-reveal-distance', label: '继续合作，但把私人关系停在这里', subtext: '工作仍然成立，信任不会自动成立。', effects: { setFlags: ['ines-revealed', 'relationship-professional'], factUpdates: [{ factId: 'fact-ines-network', state: 'revealed', sourceId: 'ines' }, { factId: 'fact-data-rights', state: 'verified', sourceId: 'ines' }], trustDelta: { ines: -1 }, memories: [{ actorId: 'ines', about: 'reveal', value: '玩家在身份隐瞒后保留距离', weight: 4 }] }, nextNodeId: 'bs-05-public' }
      ]
    },
    {
      id: 'bs-05-public', sceneId: 'scene-public', speakerId: 'player-butterfly-scholar', channel: 'record',
      text: ['临时花园开放。观众看不到一份“完整的哥斯达黎加”。他们只进入你选择留下的关系：扫描、植物、缺席、时间和一部分故意没有被公开的数据。', '这次旅行最后留下的不是一次采集任务，而是一套以后可以继续使用的方法。'],
      choices: [
        { id: 'bs-archive-open', label: '归档：保留分歧和关系', subtext: '把技术版本、身份反转和数据边界一起写入 Career Archive。', effects: { setFlags: ['butterfly-route-complete', 'archive-contradictions'] }, nextNodeId: 'bs-end' }
      ]
    },
    {
      id: 'bs-end', sceneId: 'scene-public', channel: 'record',
      text: ['ROUTE COMPLETE / 哥斯达黎加的蝴蝶学者', 'Blueprint、采集方法、人物记忆和未公开数据边界已经进入档案。'],
      choices: []
    }
  ]
};
...
export const butterflyWorldEffectsByChoice: Record<string, ButterflyWorldEffect> = {
  'bs-go': { projectTags: ['field-trip', 'ecology'], evidenceIds: ['ev-invitation'] },
  'bs-go-with-question': { projectTags: ['field-trip', 'ecology', 'preservation-question'], evidenceIds: ['ev-invitation', 'ev-question-before-trip'] },
  'bs-ask-now': { threadIds: ['thread-ines-identity'], evidenceIds: ['ev-mismatched-organization-mark'] },
  'bs-hold-question': { threadIds: ['thread-ines-identity'], evidenceIds: ['ev-mismatched-organization-mark'] },
  ...
  'bs-reveal-distance': { threadIds: ['thread-data-rights'], archiveEntryIds: ['archive-ines-reveal'], projectTags: ['data-rights'] },
  'bs-archive-open': { archiveEntryIds: ['archive-butterfly-route', 'archive-ines-reveal'], methodIds: ['method-field-metadata', 'method-selective-preservation'], evidenceIds: ['ev-public-garden-version'] }
};
