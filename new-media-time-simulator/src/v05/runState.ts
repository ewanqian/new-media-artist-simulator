...
export type Work = {
  ...
  workingTitle: string;
  projectId: string;
  status: 'draft' | 'in-progress' | 'public';
...
export type NarrativeNode =
  | { id: string; type: 'observation'; text: string; nextNodeId: string }
  | { id: string; type: 'transition'; text: string; nextNodeId: string }
...
export type NarrativeAction = {
  ...
  label: string;
  nextNodeId: string;
  result: { summary: string; delta: StateDelta };
};
...
export function createRunState(input: Pick<RunState, 'runId' | 'identity' | 'chapterId' | 'currentNodeId' | 'objective'>): RunState {
  return {
    ...
    ...input,
    resources: { cash: 8000, energy: 3, reputation: 0 },
    projects: [], works: [], people: [], evidence: [], history: [], flags: []
  };
}
...
export function isRunState(value: unknown): value is RunState {
  ...
  return Boolean(
    state && state.schemaVersion === RUN_STATE_SCHEMA_VERSION
    && typeof state.runId === 'string'
    && typeof state.currentNodeId === 'string'
    && state.resources && Number.isFinite(state.resources.cash)
    && Array.isArray(state.works) && Array.isArray(state.history)
  );
}

/** Old/untrusted saves are intentionally reset instead of guessed at. */
export function loadRunState(raw: string | null, fallback: RunState): RunState {
  if (!raw) return fallback;
  try {
    const parsed = JSON.parse(raw);
    return isRunState(parsed) ? parsed : fallback;
  } catch {
    return fallback;
  }
}

export function saveRunState(storage: Pick<Storage, 'setItem'>, state: RunState) {
  storage.setItem(V051_RUN_SAVE_KEY, JSON.stringify(state));
}
...
export function applyAction(state: RunState, node: NarrativeNode, actionId: string, now = new Date().toISOString()): RunState {
  ...
  for (const key of ['cash', 'energy', 'reputation'] as const) {
    if (typeof delta.resources?.[key] === 'number') nextResources[key] += delta.resources[key] as number;
  }
  const works = delta.addWork && !state.works.some((work) => work.id === delta.addWork?.id)
    ? [...state.works, delta.addWork] : state.works;
  return {
    ...state,
