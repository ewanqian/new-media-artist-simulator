...
export function lintContent(nodes: NarrativeNode[]): ContentLintIssue[] {
  ...
  for (const node of nodes) {
    ...
    if (node.type === 'decision') {
      ...
      for (const action of node.actions) if (!action.result.delta.addWork && !action.result.delta.addFlags?.length && !Object.keys(action.result.delta.resources || {}).length) issues.push({ nodeId: node.id, message: `Action ${action.id} has no structured consequence.` });
    }
  }
  return issues;
}
